...
def get_cached_session(email, password):
...
def fetch_products_fast(session, status_callback=None):
    ...
    while True:
        ...
        for product in products:
            tax_value = product.get("tax", None)
            ...
            costs = [pd.to_numeric(v.get("cost"), errors="coerce") for v in variants] if variants else []
            costs = [c for c in costs if pd.notna(c)]
            rows.append({
                "sku": str(product.get("sku", "") or ""),
                "titulo_catalogo": str(product.get("title", "") or ""),
                "costo_unitario": max(costs) if costs else None,
    ...
    return df.groupby("sku", as_index=False).agg({"titulo_catalogo": "first", "costo_unitario": "max", "iva": "max"})
...
def build_outputs(df_listings, df_products, status_callback=None):
    if df_listings.empty:
        detalle = pd.DataFrame(columns=["mla", "sku", "unidades", "costo_unitario", "costo_total_sku", "iva", "titulo_final"])
        final_df = pd.DataFrame(columns=["mla", "titulo_producto", "skus_asociados", "unidades_totales", "costo_total_mla", "iva", "cant_sku", "tipo_producto"])
        return detalle, final_df
    if status_callback:
        status_callback("Agrupando detalle por MLA + SKU...")
    detalle = df_listings.groupby(["mla", "sku"], as_index=False).agg({
        "unidades": "sum",
        "titulo_producto_base": lambda s: " | ".join(sorted(set([str(x) for x in s if str(x).strip()]))),
        "owner": "first",
        "account_id": "first",
    })
    if status_callback:
    ...
    detalle["titulo_final"] = detalle["titulo_producto_base"].replace("", pd.NA).fillna(detalle["titulo_catalogo"])
    detalle["costo_total_sku"] = detalle["costo_unitario"] * detalle["unidades"]
    detalle = detalle[["mla", "sku", "unidades", "costo_unitario", "costo_total_sku", "iva", "titulo_final"]].sort_values(["mla", "sku"]).reset_index(drop=True)
    if status_callback:
        status_callback("Construyendo resumen final por MLA...")
    final_df = detalle.groupby("mla", as_index=False).agg(
        titulo_producto=("titulo_final", lambda s: " | ".join(sorted(set([str(x) for x in s if str(x).strip()])))),
        skus_asociados=("sku", lambda s: " | ".join(sorted(set([str(x) for x in s if str(x).strip()])))),
        unidades_totales=("unidades", "sum"),
        costo_total_mla=("costo_total_sku", "sum"),
        iva=("iva", "max"),
        cant_sku=("sku", "nunique"),
    )
    final_df["tipo_producto"] = final_df.apply(lambda r: clasificar_mla(r["cant_sku"], r["unidades_totales"]), axis=1)
    final_df = final_df[["mla", "titulo_producto", "skus_asociados", "unidades_totales", "costo_total_mla", "iva", "cant_sku", "tipo_producto"]].sort_values(["tipo_producto", "mla"]).reset_index(drop=True)
    return detalle, final_df


@st.cache_data(show_spinner=False)
def cached_query(email, password):
    session = get_cached_session(email, password)
    df_listings = fetch_ml_listings_fast(session)
    df_products = fetch_products_fast(session)
    return build_outputs(df_listings, df_products)
...
def apply_filters():
    ...
    if fm:
        detalle_vista = detalle_vista[detalle_vista["mla"].astype(str).str.contains(fm, case=False, na=False)]
    if fs:
        detalle_vista = detalle_vista[detalle_vista["sku"].astype(str).str.contains(fs, case=False, na=False)]
    if ft:
        detalle_vista = detalle_vista[detalle_vista["titulo_final"].astype(str).str.contains(ft, case=False, na=False)]
    if tp and tp != "Todos":
        mlas_validos = set(vista["mla"].astype(str).tolist())
        detalle_vista = detalle_vista[detalle_vista["mla"].astype(str).isin(mlas_validos)]
...
with st.sidebar:
    ...
    if logout_clicked:
        for k in ["authenticated", "session_key", "df_final", "df_detalle", "df_vista", "df_detalle_vista"]:
            st.session_state[k] = False if k == "authenticated" else None if k == "session_key" else pd.DataFrame()
        ...
        if not email or not password:
            ...
            try:
                with st.spinner("Validando credenciales..."):
                    st.session_state.session_key = f"{email}:{password}"
                    st.session_state.authenticated = True
                    get_cached_session(email, password)
    ...
    if consultar:
        try:
            with st.spinner("Consultando información..."):
                t0 = time.time()
                email_key, password_key = email, password
                detalle_df, final_df = cached_query(email_key, password_key)
                st.session_state.df_detalle = detalle_df.copy()
                st.session_state.df_final = final_df.copy()
                st.session_state.status = f"Consulta completada en {round(time.time() - t0, 2)} s"
...
with filter_cols[3]:
    ...
    if not st.session_state.df_final.empty:
        tipos += sorted(st.session_state.df_final["tipo_producto"].dropna().unique().tolist())
...
with m2:
    st.metric("Costo total filtrado", round(st.session_state.df_vista["costo_total_mla"].sum(), 2) if not st.session_state.df_vista.empty else 0)
with m3:
    st.metric("Unidades totales filtradas", round(st.session_state.df_vista["unidades_totales"].sum(), 2) if not st.session_state.df_vista.empty else 0)
...
if not st.session_state.df_vista.empty:
    ...
    with c2:
        st.download_button(
            "Descargar Excel",
            data=excel_bytes(st.session_state.df_vista, st.session_state.df_detalle_vista),
